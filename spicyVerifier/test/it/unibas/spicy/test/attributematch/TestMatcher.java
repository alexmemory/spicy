...
//        String mappingTaskFile = new File(this.getClass().getResource(Riferimenti.statDBexpenseDB).toURI()).getAbsolutePath();
...
//
    public void testLivesIn() throws Exception {
        String mappingTaskFile = new File(this.getClass().getResource(Riferimenti.livesIn).toURI()).getAbsolutePath();
        if (logger.isDebugEnabled()) logger.debug("Loading mapping task...");
        mappingTask = daoMappingTask.loadMappingTask(mappingTaskFile);
